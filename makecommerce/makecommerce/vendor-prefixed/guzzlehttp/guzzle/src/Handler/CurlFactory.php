...
use MakeCommercePrefix\GuzzleHttp\Exception\ConnectException;
use MakeCommercePrefix\GuzzleHttp\Exception\RequestException;
use MakeCommercePrefix\GuzzleHttp\Promise as P;
use MakeCommercePrefix\GuzzleHttp\Promise\FulfilledPromise;
use MakeCommercePrefix\GuzzleHttp\Promise\PromiseInterface;
use MakeCommercePrefix\GuzzleHttp\Psr7\LazyOpenStream;
use MakeCommercePrefix\GuzzleHttp\TransferStats;
use MakeCommercePrefix\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \MakeCommercePrefix\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \MakeCommercePrefix\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \MakeCommercePrefix\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \MakeCommercePrefix\GuzzleHttp\Psr7\Utils::streamFor($sink);
